# Production-ready DevOps automation system using n8n, Docker, Jenkins,Telegram alerts, DevOps, LLM, Ollama, API, PDF Generation, RSS Scraping, Email Automation and JavaScript Automation.

```text
n8n-devops-automation/
│
├── docker/
│   └── docker-compose.yml
│   └── job-search-automation.json
│
├── workflows/
│   └── jenkins-alert-workflow.json
│   └── Gmail-alert-workflow.json
│   └── Telegram-alert-workflow.json
│   └── Generated-cover-letter-workflow.json
│
├── jenkins/
│   └── Jenkinsfile
│
├── docs/
│   ├── architecture.md
│   ├── setup-guide.md
│   ├── workflow-explanation.md
│   └── screenshots/
│
├── .env.example
├── README.md
└── LICENSE
```

# AI Job Search Automation using n8n + Ollama

An intelligent AI-powered job application automation workflow built with n8n.

## Features

- Automated job scraping from RSS feeds
- AI-generated personalized cover letters
- PDF generation
- Automatic recruiter email extraction
- Gmail application delivery
- Telegram notifications
- Google Sheets tracking
- Local AI using Ollama

## Technologies
- n8n
- Ollama
- Gmail API
- Telegram Bot API
- Google Sheets
- RSS Feed
- JavaScript
- Docker

## Workflow Overview
- Schedule Trigger
- RSS Feed Read
- AI Processing
- Cover Letter Generation
- PDF Generation
- Recruiter Email Extraction
- Gmail Delivery
- Telegram Notification
- Google Sheet Update

Architecture

Jenkins → n8n Webhook → Logic → Telegram Alerts

screenshots

<img width="1518" height="673" alt="image" src="https://github.com/user-attachments/assets/ffcf90a6-3a52-4d3f-88d4-a4e99aa6f685" />

<img width="1132" height="816" alt="image" src="https://github.com/user-attachments/assets/b8f838b2-0826-438e-bbab-d5d3f512e64c" />

<img width="1294" height="931" alt="image" src="https://github.com/user-attachments/assets/6cc4e767-951b-4258-850e-4b98bd763432" />

<img width="1490" height="943" alt="image" src="https://github.com/user-attachments/assets/77d1a673-dfcc-433c-bf64-e5efa0edc58b" />

Create a ChatBot on n8n with ollama 
<img width="781" height="681" alt="image" src="https://github.com/user-attachments/assets/0f4ac453-79fe-4871-9745-335cb6d6994a" />

# n8n Gmail automation with Telegram Nofitication 
  Automate your gmail account you never need to check the mail you get a message via Telegram if there is Important message.
  You never missed an Important Message again. You can also set Auto-Reply and Set cron job to check your gmail account every 2 hours and give you report update what are inside your email. Like numbers of Messages you   have in the Inbox and so on.
  <img width="809" height="684" alt="image" src="https://github.com/user-attachments/assets/743fecd5-de7a-4403-a8aa-a8e23e9df6ac" />
# Gmail Message Sent Confirmation
  <img width="1595" height="520" alt="image" src="https://github.com/user-attachments/assets/b676c2b3-8015-4ea6-b1cf-6f9a51de6392" />
  <img width="1014" height="857" alt="image" src="https://github.com/user-attachments/assets/d365ded7-edb8-4041-b79b-5b19f02ca379" />
  <img width="833" height="475" alt="image" src="https://github.com/user-attachments/assets/45c7a53b-cd51-41b8-aabe-c3fb0fd9bbd4" />




# Complete 100% n8n AI Agent Setup For Email Inbox Automation 
  <img width="1123" height="768" alt="image" src="https://github.com/user-attachments/assets/a7f44391-8280-4040-a9bc-38b6fa9f42be" />

  
# Telegram Screenshot via Email Checked Notification 
<img width="447" height="39" alt="image" src="https://github.com/user-attachments/assets/3a40b3f7-d781-4879-8780-e60a9718704b" />

# Automate Job Search with n8n 
<img width="1854" height="901" alt="image" src="https://github.com/user-attachments/assets/a3af5359-730f-4222-b134-cb21f635dfe8" />

Telegram Screenshot

<img width="489" height="230" alt="image" src="https://github.com/user-attachments/assets/ddced5dd-adcf-4bd2-96c6-f25a0153145d" />
<img width="446" height="41" alt="image" src="https://github.com/user-attachments/assets/938a05d1-1525-42f6-832b-e3c6594c6a2d" />
<img width="449" height="216" alt="image" src="https://github.com/user-attachments/assets/ceb48204-24f3-4b93-9e67-878b3507ef06" />



## Setup
1. Clone repository
2. Configure environment variables
3. Import workflow into n8n
4. Start Ollama
5. Run workflow


# Reference : 
  https://www.youtube.com/watch?v=l0SiFihbetA&t=593s
  https://www.youtube.com/watch?v=LIzZRgfW4ok&t=344s
  https://www.youtube.com/watch?v=_dZVQuQIWMs

